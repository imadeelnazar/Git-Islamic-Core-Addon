<?php
									...
									<div class="row">
										<div class="col-md-6">
											<select onchange="this.form.submit()" name="orderby" class="chosen-select">
												<option value="any">'.esc_html__('Any','jobly').'</option>';
												$events  .= '
												<option ';
												if(isset($_GET['orderby']) && $_GET['orderby'] == 'date'){$events  .= 'selected';}
												$events  .= '
												 value="date">'.esc_html__('Publish Date','jobly').'</option>';
												$events  .= '
												<option ';
												if(isset($_GET['orderby']) && $_GET['orderby'] == 'title'){$events  .= 'selected';}
												$events  .= ' value="title">'.esc_html__('Title','jobly').'</option> <option ';							
												if(isset($_GET['orderby']) && $_GET['orderby'] == 'rand'){$events  .= 'selected';}
												$events  .= ' 
												 value="rand">'.esc_html__('Random','jobly').'</option>
											</select>
										</div>
										<div class="col-md-6">
											<select onchange="this.form.submit()" name="order" class="chosen-select">
												...
												$events  .= '
												 value="asc">'.esc_html__('ASC','jobly').'</option>';
												$events  .= '
												<option ';
												if(isset($_GET['order']) && $_GET['orderby'] == 'desc'){$events  .= 'selected';}
												$events  .= ' value="desc">'.esc_html__('DESC','jobly').'</option>
											</select>
										</div>
									</div>	
								</div>
	...
	function wpha_get_events_item( $settings = array() ){
		...
		if($settings['style'] == 'event-simple-view'){
			$ret .= '<div class="wpha-listing-item kode_event_listing kode-events kode-events-classic '.esc_attr($settings['padding']).'">';
			$wpha_post_settings['thumbnail_size'] = $settings['thumbnail_size'];
		...
		foreach($array as $post_info){
			...
			switch( $post_info ){
				...
				case 'views':
					...
					$ret .= '<a href="' . esc_url(get_permalink($events_id)) . '" >' . esc_html(wpha_get_events_views($events_id)) . '<em>' . esc_html__('Views','jobly').' </em></a>';
					$ret .= '</'.esc_attr($div_wrap).'>';							
					break;		
				case 'author':
					ob_start();
	...
	function wpha_get_events_style_simple_view( $settings ){
		...
		<div class="kode_event_wrap">
			<figure>
				...
				<figcaption><a data-rel="prettyPhoto[]" href="#"> <i class="icon-add52"></i> </a></figcaption>
			</figure>
			<div class="kode_event_des">
				<h6><a href="'.esc_url($mypost->guid).'">'.esc_html(substr($mypost->post_title,0,$title_num_fetch)).'</a></h6>
				<ul>
					<li><span class="pull-left"><i class="fa fa-calendar"></i> '.__('Starts :','islamic-center').' '.date('d M Y',$mypost->start).'</span>  <span class="pull-right"><i class="fa fa-calendar"></i> '.__('Ends :','islamic-center').' '.date('d M Y',$mypost->end).'</span></li>									
				</ul>
				<p>'.esc_html(substr(strip_tags($mypost->post_content),0,150)).'</p>
				<a href="'.esc_url($mypost->guid).'">'.esc_attr__('Detail','islamic-center').'</a>
			</div>		
		</div>';
			...
			<div class="city_event2_list2_row">
				...
				<div class="city_blog_text event2">
					...
					<div class="city_blog_social">
						<a class="theam_btn border-color" href="'.esc_url($mypost->guid).'" tabindex="0">'.esc_html__('Join Event','jobly').'</a>
	...
	function wpha_get_events_style_full_view( $settings ){
		...
		<div class="city-event-item">';
			...
			$ret .= '
				...
				<a class="theam_btn btn2" href="'.esc_url($mypost->guid).'">'.esc_html__('Join Event','jobly').'</a>
			</div>
		</div>';
	...
	function wpha_get_events_style_news_view( $settings ){
		...
		<div class="city-news-event-item">
			<div class="city_news2_detail">
				...
				<p>'.esc_html(substr($mypost->post_content,0,$settings['num_excerpt'])).'</p>
				<a class="theam_btn bg-color color" href="'.esc_url($mypost->guid).'">'.esc_html__('Read More','jobly').'</a>
			</div>
		</div>';
	}
	...
	function wpha_related_events_item($post_id) {
		global $post,$forest_admin_option;				
		...
		if($tags) {
			if(isset($tags)){
				...
				if($related_posts) {
					...
					<div class="row">
					<div class="col-md-12"><h3 class="event_heading">'.esc_html__('Related Events','jobly').'</h3></div>';
						$title_num_fetch = 30;
	...
	function wpha_get_events_listing( $settings = array() ){
		...
		foreach ( $query_events as $event ) {
			...
			<div class="event-content">
				...
				<a href="'.esc_url($event->guid).'" class="btn-normal borderd white-b btn-e eft-clr-1"><span></span>'.esc_html__('See This Campaign','jobly').'</a>
			</div>';
		}
