<?php
	...
	*/	
	...
	if( !class_exists('islamic_center_page_builder_html') ){
		
		class islamic_center_page_builder_html{
		
			public $islamic_center_size = array(
			...
			function islamic_center_print_draggable_wrapper($item_atts = array()){
				echo '<div class="kode-draggable kode-draggable-wrapper ';
				echo (!empty($item_atts['size']))? $this->islamic_center_size[$item_atts['size']] : $this->islamic_center_size['1/1'];
				...
				if( !empty($item_atts['inner-item']) ){
					$this->islamic_center_print_page_builder($item_atts['inner-item']);
				}
				echo '</div>';
				...
				if(!empty($item_atts['options'])){
					$this->islamic_center_print_item_option($item_atts['options']);
				}
				echo '</div>'; // kode-draggable-wrapper			
			}
			
			// print the draggable item type
			function islamic_center_print_draggable_item($item_atts = array()){
				echo '<div class="kode-draggable kode-draggable-item ' . esc_attr($this->islamic_center_size['1/1']) . '" ';
				echo 'data-type="' . esc_attr($item_atts['slug']) . '" ';
				echo '>';
				echo '<div class="kode-item">';
			...
			function islamic_center_print_item_option($options){
			...
			function islamic_center_print_page_builder( $items = array() ){
				...
				foreach( $items as $item ){
					if( $item['item-type'] == 'wrapper' ){
						...
						$this->islamic_center_print_draggable_wrapper( $current_item );	
					}else if( $item['item-type'] == 'item' ){
						$this->islamic_center_print_page_builder_item( $item ); 
					}
				}
			}
			
			// print page builder item from save data
			function islamic_center_print_page_builder_item( $item ){
				$current_item = $this->items[ $item['type'] ];
				$current_item['slug'] = $item['type'];
				$current_item['item-builder-title'] = empty($item['item-builder-title'])? '': $item['item-builder-title'];
				...
				$this->islamic_center_print_draggable_item($current_item);			
			}
